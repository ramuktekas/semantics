...
cosine_file_path = 'A1_ran_177subs.csv'
worddepth_file_path = 'TA2_ran_177subs.csv'
PSL_file_path = 'PSL_ran_177subs.csv'
...
cosine_similarity_timeseries = cosine_data['A1_ran'].values
word_depth_timeseries = worddepth_data['TA2_ran'].values
PSL_timeseries = PSL_data['PSL_ran'].values
...
axs[1].set_ylim(2.3,3.8)
axs[1].set_title('Dynamic ACW-0 of TA2 (Rest)', fontweight='bold')

# Add rest intervals as grey boxes
for rest in rest_intervals:
    axs[1].axvspan(rest[0], rest[1], color='grey', alpha=0.3)
#for silence in silent_intervals:
...
axs[2].set_ylim(2.3, 3.8)
axs[2].set_title('Dynamic ACW-0 of PSL (Rest)',fontweight='bold')
...
# Add rest intervals as grey boxes
for rest in rest_intervals:
    axs[2].axvspan(rest[0], rest[1], color='grey', alpha=0.3)
#for silence in silent_intervals:
...
save_path_line_plots = os.path.join(save_directory, 'DY_ACW_REST.png')
plt.tight_layout()
